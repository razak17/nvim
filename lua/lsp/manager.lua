...
local function resolve_config(name, user_config)
  local config = {
    on_attach = require("lsp").global_on_attach,
    on_init = require("lsp").global_on_init,
    capabilities = require("lsp").global_capabilities(),
  }
...
function M.setup(server_name, user_config)
  ...
  local function ensure_installed(server)
    ...
    if not rvim.lsp.automatic_servers_installation then
