...
function fs:render_window()
  ...
  local opts = {relative = "editor", height = 1, row = row, col = col}

  local border_opts = {border = 1, title = 'ProJect'}

  local header_opts = {contents = self.contents, filetype = 'ProJectHeader'}

  self.hcb, self.hcw, _, self.hbw = window.create_float_window(header_opts,
                                                               border_opts, opts)
  ...
  local input_border_opts = {border = 1}
  ...
  self.input_cb, self.input_cw, _, self.input_bw =
      window.create_float_window(input_content_opts, input_border_opts,
                                 input_opts)
  ...
  api.nvim_buf_add_highlight(self.input_cb, file_input_prefix,
                             'FileInputPrefix', 0, 0, #self.prompt_prefix)
  vim.cmd [[startinsert!]]
  api.nvim_command(
      'inoremap <buffer><nowait><silent> <cr> <cmd>lua require("internal.fsevent").do_file_action()<CR>')
  api.nvim_command(
...
function fs:quit_fs_window()
  ...
  window.nvim_close_valid_window({
    self.input_cw,
    self.input_bw,
    self.hcw,
    self.hbw
