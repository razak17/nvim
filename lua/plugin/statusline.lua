...
function M.setup()
  local func = [[
  function! Status()
    return luaeval("require 'plugins/statusline'.update()")
  endfunction

  augroup StatusLine
    au!
    au TabEnter,WinLeave,BufEnter,WinEnter,VimEnter * lua require'plugins/statusline'.clear()
    au BufEnter,VimEnter * setlocal statusline=%!Status()
    au DirChanged * lua require'plugins/statusline'.update_git()
  augroup END
  ]]
  api.nvim_exec(func, '')
