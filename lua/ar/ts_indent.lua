-- https://github.com/nvim-treesitter/nvim-treesitter/blob/4916d6592ede8c07973490d9322f187e07dfefac/lua/nvim-treesitter/indent.lua?plain=1#L1
...
local function find_delimiter(bufnr, node, delimiter)
  for child, _ in node:iter_children() do
    if child:type() == delimiter then
      ...
      local line =
        vim.api.nvim_buf_get_lines(bufnr, linenr, linenr + 1, false)[1]
      local end_char = { child:end_() }
      local trimmed_after_delim
      local escaped_delimiter =
        delimiter:gsub('[%-%.%+%[%]%(%)%$%^%%%?%*]', '%%%1')
      trimmed_after_delim = assert(line)
        :sub(end_char[2] + 1)
        :gsub('[%s' .. escaped_delimiter .. ']*', '')
...
local get_indents = memoize(
  ...
  function(bufnr, root, lang) return tostring(bufnr) .. root:id() .. '_' .. lang end
)
  ...
  while node do
    ...
    if
      ...
      and (
        ...
        and (
          srow ~= lnum - 1
          or q['indent.begin'][node:id()]['indent.start_at_same_line']
        )
      )
    then
      indent = indent + indent_size
    ...
    then
      ...
      if o_delim_node then
        ...
        if c_delim_node then c_srow = c_delim_node:start() end
        ...
        if c_srow and c_srow ~= o_srow and c_srow == lnum - 1 then
          ...
          avoid_last_matching_next = metadata['indent.avoid_last_matching_next']
            or false
        end
