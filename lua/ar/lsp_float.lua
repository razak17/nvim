...
local function set_float_hl(buf, win)
  ...
  if ok then
    api.nvim_set_hl(ns, "@text.reference", { fg = c.blue, underline = true })
  ...
  for l, line in ipairs(api.nvim_buf_get_lines(buf, 0, -1, false)) do
    for pattern, hl_group in pairs(hls) do
      ...
      while from do
        ...
        if from then
          api.nvim_buf_set_extmark(buf, ns, l - 1, from - 1, {
            end_col = to,
            hl_group = hl_group,
          })
        end
...
local function set_float_keymaps(buf)
  local function opener()
    ...
    local tag = (fn.expand("<cWORD>")):match("|(%S-)|")
    if tag then
    ...
    local from, to, url = api.nvim_get_current_line():find("%[.-%]%((%S-)%)")
