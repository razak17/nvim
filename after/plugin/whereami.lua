...
local ease = function(ease, from, to, position)
  ...
  elseif ease == 'ease-in-out-quad' then
    easeValue = position < 0.5 and 2 * (y ^ 2)
      or 1 - (((-2 * position + 2) ^ 2) / 2)
  ...
  elseif ease == 'ease-in-out-cubic' then
    easeValue = position < 0.5 and 4 * (position ^ 3)
      or 1 - (((-2 * position + 2) ^ 3) / 2)
  ...
  elseif ease == 'ease-in-out-quart' then
    easeValue = position < 0.5 and 8 * (position ^ 4)
      or 1 - (((-2 * position + 2) ^ 4) / 2)
  ...
  elseif ease == 'ease-in-out-quint' then
    easeValue = position < 0.5 and 16 * (position ^ 5)
      or 1 - (((-2 * position + 2) ^ 5) / 2)
  ...
  elseif ease == 'ease-in-out-circ' then
    easeValue = position < 0.5 and (1 - math.sqrt(1 - ((2 * y) ^ 2))) / 2
      or (math.sqrt(1 - ((-2 * y + 2) ^ 2)) + 1) / 2
  end
...
api.nvim_create_user_command('Beacon', function(options)
  ...
  startColors, animationValues = gradient(
    color,
    rgbToTable(normalBg),
    colCount,
    animationSteps,
