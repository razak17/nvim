local enabled = ar_config.plugin.custom.git_conflict.enable

if not ar or ar.none or not enabled then return end

-- https://github.com/chrisgrieser/.config/blob/main/nvim/lua/personal-plugins/git-conflict.lua?plain=1#L1
...
local function map(bufnr, key, rhs, desc)
  ...
  vim.keymap.set('n', lhs, function()
    if
      key == keys.gotoMarker or vim.api.nvim_get_current_line():find('<<<<*')
    then
      return rhs
    end

    local msg = 'Needs to be on the 1st merge marker.\n'
      .. ('Use `%s` to go to the next marker.'):format(
        keys.leader .. keys.gotoMarker
      )
    notify(msg, 'warn')
...
local function setupConflictMarkers(out, bufnr)
  ...
  if #conflictLnums % 4 ~= 0 then
    notify(
      'Conflicts found but conflict style is not `diff3` or `zdiff3`. Aborting.',
      'warn'
    )
    return
  end
  ...
  for i = 1, #conflictLnums do
    ...
    vim.hl.range(
      bufnr,
      ns,
      config.hlgroups.borders,
      { lnum, 0 },
      { lnum, gitMarkerLength }
    )
  ...
  vim.g.whichkey_add_spec({ '<leader>gk', group = 'Conflict' })
