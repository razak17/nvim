...
-- Ref: https://www.reddit.com/r/neovim/comments/1wcafcg/vimdirvish_to_dirlua/
...
map('n', '.', function()
  ...
  vim.api.nvim_feedkeys(
    vim.api.nvim_replace_termcodes(
      ':! ' .. vim.fn.fnameescape(fname) .. '<Home><Right>',
      true,
      false,
      true
    ),
...
map('n', 'r', function()
  ...
  local fname = vim.api.nvim_buf_get_lines(0, cursor[1] - 1, cursor[1], true)[1]
  if fname == '' then return end
  local ok, fname_new = pcall(
    vim.fn.input,
    { prompt = "Rename '" .. fname .. "' to: ", default = fname }
  )
  if not ok or fname_new == '' then return end
  ...
  if not res then
    vim.notify(
      "Failed to unload open buffer(s) for '" .. fname .. "'",
      vim.log.levels.ERROR
    )
    return
  end
...
map('n', 'dd', function()
  ...
  if fname == '' then return end
  local ok, confirm =
    pcall(vim.fn.input, { prompt = "Delete '" .. fname .. "'? [Y/n] " })
  if not ok or not (confirm:lower() == 'y' or confirm == '') then return end
  local full_path = vim.fs.joinpath(vim.uv.cwd(), fname)
  local res = run_on_buf(function(bufnr)
    if bufnr ~= -1 and vim.api.nvim_buf_is_loaded(bufnr) then
      vim.api.nvim_buf_delete(bufnr)
