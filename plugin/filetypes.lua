...
settings({
  ...
  [{
    ...
    function()
      ...
      if not ar.falsy(override) and vim.tbl_contains(override, 'ts_ls') then
        goto continue
      end
      if not ar_config.lsp.lang.typescript['ts_ls'] then return end
      ...
      local function ts_cmd(name, cb, desc)
        api.nvim_buf_create_user_command(0, name, cb, { desc = desc })
      end
